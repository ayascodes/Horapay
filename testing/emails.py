...
def send_activation_email(email,password):
    ...
    user_obj.save()
def send_activation_email_admin(email,password):
    subject="Votre compte admin HoraPay"
    login_url=f"http://127.0.0.1:8000/testing/token/"
    message = f"Bonjour,\n\nVotre compte admin HoraPay a été créé avec succès. Voici vos informations de connexion :\n\nEmail: {email}\nMot de passe: {password}\n\nVous pouvez vous connecter à votre compte en utilisant le lien suivant :\n{login_url}\n\nCordialement,\nL'équipe HoraPay"
    email_from = settings.EMAIL_HOST
    send_mail(subject,message,email_from,[email])
    user_obj = CustomUser.objects.get(email=email)
    user_obj.save()
